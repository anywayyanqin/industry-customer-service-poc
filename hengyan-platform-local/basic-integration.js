(function () {
  ...
  function addBasicApplications() {
    ...
    center.querySelectorAll('[data-open-basic-workbench]').forEach(function (b) { b.onclick=function(){navigateBasic('/basic-service/index.html#workbench');}; });
    if (!center.querySelector('.native-course-strip')) {
      ...
      courses.querySelector('button').onclick=function(){navigateBasic('/basic-service/training-center.html#course/hedge-accounting');}; center.appendChild(courses);
    }
  }
  function addBasicFollow() {
    ...
    card.querySelector('button').onclick=function(){navigateBasic('/basic-service/basic-service-home.html#content/policy-center');}; center.appendChild(card);
  }
  function addBasicActivity() {
    ...
    card.querySelector('button').onclick=function(){navigateBasic('/basic-service/activity-detail.html?id=policy-live');}; center.appendChild(card);
  }
  ...
  function renderBasicWorkspace(){
    ...
    box.querySelector('[data-basic-home]').onclick=function(){navigateBasic('/basic-service/basic-service-home.html');};
    box.querySelectorAll('[data-basic-go]').forEach(function(b){b.onclick=function(){navigateBasic(b.dataset.basicGo);};});
  }
  function loadBasic(path) {
    if (/\/basic-service\/(?:index|basic-service-home|consulting-center|training-center|service-hubs|hedge-accounting-(?:apply|service|result))\.html/.test(path) && path.indexOf('v=20260918-9') < 0) {
      var hashIndex = path.indexOf('#');
      var pagePath = hashIndex < 0 ? path : path.slice(0, hashIndex);
      var hash = hashIndex < 0 ? '' : path.slice(hashIndex);
      path = pagePath + (pagePath.indexOf('?') < 0 ? '?' : '&') + 'v=20260918-9' + hash;
    }
    document.dispatchEvent(new CustomEvent('open-migrated-basic', { detail: { path: path } }));
  }
  function navigateBasic(path) {
    var basic = Array.prototype.find.call(document.querySelectorAll('.main-header nav button'), function (button) {
      return button.textContent.trim() === '基础服务';
    });
    if (!basic) {
      loadBasic(path);
      return;
    }
    basic.dataset.formalBasicPath = path;
    basic.click();
  }
  ...
  function bindBasicNavigation() {
    var basic = Array.prototype.find.call(document.querySelectorAll('.main-header nav button'), function (b) { return b.textContent.trim() === '基础服务'; });
    if (!basic || basic.dataset.formalBasicBound === '1') return;
    basic.dataset.formalBasicBound = '1';
    basic.addEventListener('click', function () {
      var path = basic.dataset.formalBasicPath || '/basic-service/basic-service-home.html';
      delete basic.dataset.formalBasicPath;
      loadBasic(path);
    }, true);
  }
  setInterval(mount, 500);
  setInterval(mountNativeWorkspace, 500);
  setInterval(bindBasicNavigation, 700);
